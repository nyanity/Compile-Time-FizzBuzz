...
static constexpr auto fizzbuzz{ detail::fizzbuzz_impl<N>{} };

template<std::size_t Start, std::size_t End,
    typename = std::enable_if_t<(((End - Start) > 0) and (Start > 0) and (End > 0))>>
static constexpr auto fizzbuzz_interval{ detail::fizzbuzz_interval_impl<Start, End>{} };

template<std::size_t Index,
    typename = std::enable_if_t <(Index > 0)>>
static constexpr auto fizzbuzz_at{ detail::fizzbuzz_at_impl<Index>{} };
